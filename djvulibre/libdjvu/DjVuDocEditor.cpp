		...
		else
		{
			...
			if (singlepage)
			{
				...
				if (file_pool)
				{
					...
					const GP<ByteStream> gstr_out(ByteStream::create(save_doc_url, std::ios_base::binary | std::ios_base::out));
			...
			else if (save_doc_type == INDIRECT)
			{
				...
				DataPool::load_file(save_doc_url);
				const GP<ByteStream> gstr(ByteStream::create(save_doc_url, std::ios_base::binary | std::ios_base::out));
				const GP<IFFByteStream> giff(IFFByteStream::create(gstr));
				IFFByteStream &iff = *giff;
