	...
	*/
	...
	public:
		...
		/** Sets the current position for reading or writing the ByteStream.  Class
			...
			exception. */
		virtual int seek(long offset, int whence = std::ios_base::beg, bool nothrow = false); // SEEK_SET
	...
	private:
		...
		ByteStream & operator=(const ByteStream &) { throw; };
	public:
		...
		static GP<ByteStream> create(
			const GURL &url, int mode = std::ios_base::binary | std::ios_base::in);
