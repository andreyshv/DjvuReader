...
namespace DJVU {
	...
	public:
		...
		/** Constructs a ByteStream for accessing the file named #url#.
			...
			known stdio function #fopen#. This constructor will open a stdio file and construct a
			...
			//GUTF8String init(const GURL &url, int mode);
		void init(const GURL &url, int mode);
		...
		virtual int seek_(long offset, std::ios::seekdir whence = std::ios_base::beg, bool nothrow = false);
	...
	public:
		...
		virtual size_t write(const void *buffer, size_t size);
		//virtual int    seek(long offset, int whence = SEEK_SET, bool nothrow = false);
		virtual int seek_(long offset, std::ios::seekdir whence = std::ios_base::cur, bool nothrow = false);
		virtual long   tell(void) const;
	...
	static FILE*
	urlfopen(const GURL &url, int mode)
	{
		...
		if (filename.ncopy(wfilename, wfilename_size) > 0)
		{
			...
			{
				retval = new std::fstream();
				retval->open(wfilename, wmode);
			}
		}
	...
	void
		ByteStream::Stdio::init(const GURL &url, int mode)
	{
		fp->open(url.UTF8Filename().toString(), mode);
		if (fp->bad())
		{
			//  Failed to open '%s': %s
		...
		can_read = mode & std::ios::in;
		can_write = mode & std::ios::out;
	}
	...
	int
		ByteStream::Stdio::seek_(long offset, std::ios::seekdir whence, bool nothrow)
	{
		//if (whence == SEEK_SET && offset >= 0 && offset == tell())
		//	return 0;

		fp->clear();
		fp->seekg(offset, whence);
		...
		ByteStream::Memory::init(void const * const buffer, const size_t sz)
	{
		...
		catch (const GException &ex) { // The only error that should be thrown is out of memory...
			retval = ex.get_cause();
		};
		return retval;
	}
	...
	int
		ByteStream::Memory::seek_(long offset, std::ios::seekdir whence, bool nothrow)
	{
		int nwhere = 0;
		switch (whence)
		{
		case std::ios_base::beg: nwhere = 0; break; //case SEEK_SET: nwhere = 0; break;
		case std::ios_base::cur: nwhere = where; break; //case SEEK_CUR: nwhere = where; break;
		case std::ios_base::end: nwhere = bsize; break; //case SEEK_END: nwhere = bsize; break;
	...
	GP<ByteStream>
	ByteStream::create(const GURL &url, int xmode)
	{
		GP<ByteStream> retval;
		//const char *mode = ((xmode) ? xmode : "rb");
		int mode = (xmode) ? xmode : std::ios_base::binary | std::ios_base::in;
		if (!retval)
		{
			Stdio *sbs = new Stdio();
			retval = sbs;
